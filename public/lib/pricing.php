...
function price_map_for_unit(string $root, string $unit): array {
  $j = load_json("$root/units/$unit/prices.json");
  $map = [];
  if (!$j) return $map;
  ...
  if (isset($j['daily']) && is_array($j['daily'])) {
    foreach ($j['daily'] as $d => $p) {
      if (preg_match('/^\d{4}-\d{2}-\d{2}$/',$d)) $map[$d] = (float)$p;
    }
    return $map;
  }
  ...
  foreach ($j as $k => $v) {
    if (preg_match('/^\d{4}-\d{2}-\d{2}$/',$k)) { $map[$k] = (float)$v; $hasDirectDates = true; }
  }
  if ($hasDirectDates) return $map;

  // c) { "prices": [ {date,price} | {start,end,price} ] }
  $list = $j['prices'] ?? null;
  if (!$list && isset($j[0]) && is_array($j[0])) $list = $j; // legacy: root je seznam
  if (is_array($list)) {
    foreach ($list as $row) {
      if (isset($row['date'])) {
        $map[$row['date']] = (float)($row['price'] ?? 0);
      } elseif (isset($row['start'],$row['end'])) {
        $d = new DateTimeImmutable($row['start'].' 00:00:00', new DateTimeZone('UTC'));
        $e = new DateTimeImmutable($row['end']  .' 00:00:00', new DateTimeZone('UTC'));
        while ($d < $e) {
          $map[$d->format('Y-m-d')] = (float)($row['price'] ?? 0);
          $d = $d->modify('+1 day');
        }
      }
    }
...
function busy_set_for_unit(string $root, string $unit): array {
  $j = load_json("$root/occupancy.json");
  $busy = [];
  if (!$j) return $busy;
  foreach ($j as $row) {
...
function span_is_clear(string $root, string $unit, string $from, string $to, array $priceMap=null, array $busySet=null): bool {
  if ($priceMap === null) $priceMap = price_map_for_unit($root, $unit);
  if ($busySet  === null) $busySet  = busy_set_for_unit($root, $unit);
  foreach (iso_datespan($from,$to) as $d) {
    if (!isset($priceMap[$d])) return false;
    if (!empty($busySet[$d])) return false;
  }
  return true;
}
